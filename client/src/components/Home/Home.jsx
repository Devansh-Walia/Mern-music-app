...
function Home({ user }) {
  ...
  useEffect(() => {
    ...
    axios
      .get("/getartists")
      .then((response) => {
        setArtistcollection(response.data);
      })
      .catch((err) => console.log(err));
    axios
      .get("/getpunartists")
      .then((response) => {
        setpunArtistcollection(response.data);
      })
      .catch((err) => console.log(err));
    axios
      .get("/gethinartists")
      .then((response) => {
        sethinArtistcollection(response.data.reverse());
      })
      .catch((err) => console.log(err));
    axios
      .get("/getgenartists")
      .then((response) => {
        setgenArtistcollection(response.data);
      })
      .catch((err) => console.log(err));
    axios
      .get("/getplaylists")
    ...
    axios
      .get("/getsongs")
      .then((response) => {
        setSongcollection(response.data);
      })
      .catch((err) => console.log(err));
  ...
  useEffect(() => {
    ...
    axios
      .post("/likedmusic", { ids: user.likedby })
      .then((response) => {
        console.log(response);
        setlikecollection(response.data);
      })
      .catch((err) => console.log(err));
  }, [user]);
        ...
        <div className="container-fluid" style={{ height: "100vh" }}>
            ...
            <div>
              {user !== "" ? (
                <div style={{ float: "right" }}>
                  ...
                  <p className="username" style={{ float: "right" }}>
          ...
          <div className="row cendiv">
            <div className="col-12" style={{ paddingRight: "0" }}>
              <div className="main-content">
                <div className="container-fluid" style={{ paddingLeft: "0" }}>
                  {mode === "HOME" && (
                    <div className="custom-black topinfotop w-100 d-flex justify-content-between align-items-center p-2 text-white mt-5 rounded-2">
                      <div className="w-25 topinfotop">
                        <img
                          className="w-100 topinfotop"
                          id="topinfoimage"
                          src={collection[current_song].img}
                          alt="NOT AVAILABLE"
                        ></img>
                      </div>
                      <div className="d-flex topinfotop flex-column w-50 align-items-end pe-3 justify-content-around">
                        <p className="fs-3 topinfotop topinfotop1 m-0 p-0">
                          {collection[current_song].songName}
                        </p>
                        <p className="fs-5 topinfotop topinfotop2 m-0 p-0">
                  ...
                  {mode === "QUEUE" && (
                    <div style={{}}>
                      <Queue
                        ...
                        setCurrent_song={setCurrent_song}
                        audioRef={audioRef}
                  ...
                  {mode === "PROFILE" && user !== "" && (
                    <Profile
                      user={user}
                      likecollection={likecollection.length}
                    ></Profile>
                  )}
                  {mode === "SHARE" && (
                    <div className="container dvdrow">
                      ...
                      <div className="row">
                        ...
                        <div className="col-md-6">
                          <Rightinfo
                            isPlaying={isPlaying}
                            currname={collection[current_song].songName}
                          ></Rightinfo>
                        </div>
                      </div>
                    </div>
              ...
              <Navbar
                current_song={collection[current_song]}
                mode={mode}
                setmode={setmode}
              ></Navbar>
            </div>
          ...
          <div
            className="row position-fixed bottom-0 start-0 end-0 bottomNav"
            style={{ height: "100px" }}
          >
            <div className="col-12">
              <div className="container-fluid no-gutters">
                <div className="row botdiv w-100 m-0 p-0">
                  <div className="col-sm-2 w-100"></div>
